...
const ShopPage = () => {
  ...
  // Mobile view filter close when click somewhere
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (filters.gender !== "all" && filters.productType !== "wall_clock")
  ...
  useEffect(() => {
    ...
    // ✅ FIXED: Hide wall clocks when gender filter is applied (except "all")
    if (filters.gender !== "all") {
      result = result.filter(
        (p) => p.productType !== "wall_clock" && p.gender === filters.gender,
      );
    }
  ...
  return (
    <div className="bg-black text-white min-h-screen">
      ...
      {/* ================= MAIN CONTENT ================= */}
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-5 sm:py-8 lg:py-10">
        ...
        <div className="mb-3 sm:mb-4">
          <div className="relative w-full max-w-xl mx-auto sm:mx-0">
            <input
              ...
              className="w-full bg-gray-900/60 border border-gray-700 rounded-full pl-10 sm:pl-12 pr-4 py-2.5 sm:py-3 text-sm sm:text-base text-white focus:outline-none focus:border-gold focus:ring-1 focus:ring-gold"
            />
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-4 w-4 sm:h-5 sm:w-5 absolute left-3.5 sm:left-4 top-1/2 transform -translate-y-1/2 text-gray-400"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        {/* ===== CONTROLS ROW: Filter btn + View Toggle + Sort — all in one line on mobile ===== */}
        <div className="flex items-center gap-2 sm:gap-4 mb-4 sm:mb-6 lg:mb-8">

          {/* Filter Button — mobile only */}
          <button
            onClick={() => setShowFilters(true)}
            className="lg:hidden bg-gray-900 border border-gray-700 px-3 py-2 rounded-lg text-gold flex items-center gap-1.5 flex-shrink-0 text-sm active:scale-95"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-4 w-4"
          ...
          {/* View Toggle */}
          <div className="flex bg-gray-900/60 rounded-lg border border-gray-700 p-0.5 sm:p-1 flex-shrink-0">
            <button
              onClick={() => setViewMode("grid")}
              className={`p-1.5 sm:p-2 rounded ${viewMode === "grid" ? "bg-gold text-black" : "text-gray-400 hover:text-white"}`}
            ...
            <button
              ...
              className={`p-1.5 sm:p-2 rounded ${viewMode === "list" ? "bg-gold text-black" : "text-gray-400 hover:text-white"}`}
          ...
          <select
            ...
            className="bg-gray-900/60 border border-gray-700 rounded-lg px-2 sm:px-3 py-2 text-xs sm:text-sm text-gray-300 focus:outline-none focus:border-gold flex-1 sm:flex-none min-w-0"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* ===== MOBILE FILTER OVERLAY (fixed drawer from left) ===== */}
          {showFilters && (
            <div className="fixed inset-0 z-50 lg:hidden">
              ...
              <aside
                ref={filterRef}
                className="absolute top-0 left-0 h-full w-72 max-w-[85vw] bg-gray-950 border-r border-gray-800 shadow-2xl overflow-y-auto z-10"
              >
                <div className="p-4 sticky top-0 bg-gray-950 border-b border-gray-800 flex justify-between items-center">
                  <h3 className="font-semibold text-white">Filters</h3>
                  <button
                    onClick={() => setShowFilters(false)}
                    className="text-gray-400 hover:text-white p-1 active:scale-95"
                  >
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="pb-4 border-b border-gray-800/50">
                    <h4 className="text-gray-400 text-xs uppercase mb-3 tracking-wider">
                      Our Premium Brands
                    </h4>
                        ...
                        >
                          {logo ? (
                            <img
                              src={logo}
                              alt={`${brand} Logo`}
                              className="h-12 w-auto object-contain filter hover:scale-105 transition-all duration-300 rounded-lg"
                              onError={(e) => (e.target.src = "/placeholder.png")}
                            />
                          ) : (
                            <div className="h-12 w-24 flex items-center justify-center bg-gray-800 rounded text-xs text-gold">
                              {brand}
                            </div>
                          )}
                          <span className="text-[11px] text-gray-400 mt-1">{brand}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full bg-gray-800 hover:bg-gray-700 py-2.5 rounded-lg text-sm active:scale-95"
                  >
                    Reset Filters
                  </button>
                </div>
                ...
                {[
                  {
                    label: "Brand",
                    name: "brand",
                    options: ["all", ...getUniqueBrands()],
                  },
                  {
                    label: "Shape",
                    name: "shape",
                    options: ["all", ...getUniqueShapes()],
                  },
                  {
                    label: "Color",
                    name: "color",
                    options: ["all", ...getUniqueColors()],
                  },
                ].map(({ label, name, options }) => (
                  <div key={name}>
                    <label className="text-gray-400 text-xs mb-1 block">
          ...
          <main className="flex-1">
            ...
            {!loading && error && <p className="text-red-400 py-10">{error}</p>}

            {!loading && !error && filteredProducts.length === 0 && (
              <div className="border border-gray-800 rounded-2xl p-6 sm:p-10 text-center bg-gray-900/30">
                <div className="text-gold text-4xl sm:text-5xl mb-3 sm:mb-4">⌚</div>
            ...
            {!loading && !error && filteredProducts.length > 0 && (
              <div
                className={
                  viewMode === "grid"
                    ? "grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2.5 sm:gap-4 lg:gap-6"
                  ...
                  return (
                    <Link
                      ...
                      className={`bg-gray-900/40 border border-gray-800 rounded-xl overflow-hidden hover:border-gold transition ${
                        viewMode === "list" ? "flex gap-3 sm:gap-6 p-2.5 sm:p-4" : ""
                      }`}
                    >
                      {/* Image */}
                      <div
                      ...
                      {/* Content */}
                      <div className={viewMode === "list" ? "flex-1 min-w-0" : "p-2.5 sm:p-4"}>
                        <div className="flex justify-between items-start mb-1.5 sm:mb-2 gap-1">
                          <h3 className="font-semibold text-xs sm:text-sm lg:text-base line-clamp-2 sm:line-clamp-1">
                            {product.title}
                          </h3>
                          <span className="text-gold text-xs sm:text-sm font-medium whitespace-nowrap flex-shrink-0">
                            {formatPrice(product.price)}
                          </span>
                        </div>

                        <span className="text-gray-400 text-xs uppercase mb-1.5 sm:mb-2 block">
                          {product.brand}
                        </span>

                        <p
                          className={`text-gray-400 text-xs mb-2 sm:mb-3 ${viewMode === "list" ? "line-clamp-2" : "line-clamp-2"}`}
                        >
                          {product.description}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-1 mb-2 sm:mb-3">
                          <span
                            className={`text-[10px] px-1.5 sm:px-2 py-0.5 rounded ${getGenderBadgeClass(product.gender, product.productType)}`}
                          >
                            {getGenderDisplay(
                              product.gender,
                              product.productType,
                          ...
                          {productColors.length > 0 && (
                            <span className="text-[10px] px-1.5 sm:px-2 py-0.5 rounded bg-gray-800 text-gray-300">
                        ...
                        <div className="mt-auto">
                          <span className="text-gold hover:text-yellow-300 text-xs sm:text-sm font-medium flex items-center gap-1 group-hover:gap-2 transition">View Details</span>
                        </div>
                      </div>
                    </Link>
                  );
